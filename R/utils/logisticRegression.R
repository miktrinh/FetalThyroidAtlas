...
#' @param workers How many parallel threads to use.  Upon failure due to memory exhaustion, workers will be halved until success is reached.
...
trainModel = function(refMat,classes,lengths=rep(1,nrow(refMat)),maxCells=2000,minCells=100,workers=multicoreWorkers(),...){
  ##############
  # Sub-sample #
  ##############
  ...
  while(TRUE){
    fit = tryCatch(multinomialFitCV(dat,classes[w],workers=workers,...),
                   error = function(e) NULL)
    if(!is.null(fit))
      break
    if(is.null(workers))
      stop("Fitting failed.")
    workers = ceiling(workers/2)
    if(workers==1)
      workers=NULL
  }
  #####################
...
multinomialFitCV = function(x,y,alpha=0.99,workers=multicoreWorkers(),verbose=is.null(workers) || workers==1,...){
                            ...
                            c(
                              list(X=marks,
                                   ...
                                   FUN = function(mark,...){
                                     ...
                                     tmp = tryCatch(
                                                    cv.glmnet(x,fac,offset=getPopulationOffset(fac),family='binomial',intercept=FALSE,alpha=alpha,type.measure='class',...),
